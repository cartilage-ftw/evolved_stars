...
def compare_spectral_lines(ep1_line_path, ep2_line_path, line_name='', axis='velocity'):
    """
    axis - 'velocity' or 'freq'
    """

    freq_axis = 'Velocity'
    # whether to plot velocity or frequency on x-axis
    if axis == 'freq':
        freq_axis = 'Frequency'
    ...
    ax.fill_between(ep1_data[freq_axis], ep1_data['Flux Density'], color='dodgerblue',
                zorder=2, alpha=0.6, step='pre', label='Nov 2015')
    ax.fill_between(ep2_data[freq_axis], ep2_data['Flux Density'],
                alpha=0.6, step='pre', color='gold', label='Nov 2017')

    #ax.step(ep1_data[freq_axis], ep1_data['Flux Density'], c='k', lw=0.25)
    ax.step(ep2_data[freq_axis], ep2_data['Flux Density'], c='r', lw=0.25, alpha=0.4)
...
if __name__ == '__main__':
    ...
    for ep1_file, ep2_file, line_name in zip(ep1_files, ep2_files, line_names):
        compare_spectral_lines(ep1_path + ep1_file, ep2_path + ep2_file, line_name, axis='velocity')
        # and the absorption regions
        ep1_abs = ep1_file[:-6] + '_abs.ascii'
        ep2_abs = ep2_file[:-6] + '_abs.ascii'
        ep1_pt = ep1_file[:-6] + '_pt_profile.ascii'
        ep2_pt = ep2_file[:-6] + '_pt_profile.ascii'
