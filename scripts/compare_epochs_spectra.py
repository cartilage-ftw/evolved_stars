...
def compare_spectral_lines(ep1_line_path, ep2_line_path, line_name='', axis='velocity'):
    ...
    ax.fill_between(ep1_data[freq_axis]/1E3, ep1_data['Flux Density']*1E3, color='dodgerblue',
                zorder=2, alpha=0.6, step='pre', label='Nov 2015')
    ax.fill_between(ep2_data[freq_axis]/1E3, ep2_data['Flux Density']*1E3,
                alpha=0.6, step='pre', color='gold', label='Nov 2017')

    #ax.step(ep1_data[freq_axis], ep1_data['Flux Density'], c='k', lw=0.25)
    ax.step(ep2_data[freq_axis]/1E3, ep2_data['Flux Density']*1E3, c='r', lw=0.25, alpha=0.4)
...
if __name__ == '__main__':
    ...
    line_names = ['H$_2$O 331.1273 GHz', '$^{29}$SiO $v=5$', '$^{12}$CO $v=1$',
                'SiO $v=2$', 'SiO $v=7$',
                 'SiO $v=7$ neighbours'] #'SiO $v=1$', 'H$^{13}$CN', 'SO $3\Sigma=0$']
    ...
    for ep1_file, ep2_file, line_name in zip(ep1_files, ep2_files, line_names):
        ...
        #ep1_pt = ep1_file + '_pt.profile'
        #ep2_pt = ep2_file + '_pt.profile'
